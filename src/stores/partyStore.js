...
export const usePartyStore = defineStore('party', () => {
  ...
  async function getAvailableContributions() {
    ...
    if (error) {
      ...
      const { data: attendeeContributions } = await supabase
        .from('attendee_contributions')
        .select('*')
      ...
      availableContributions.forEach((category) => {
        category.contributions.forEach((contribution) => {
          ...
          const existingQty = attendeeContributions.filter(
            (ac) => ac.contribution_id === contribution.id
          ).length
  ...
  async function submit() {
    ...
    try {
      ...
      const { data: currentContributions, error } = await supabase.from('contributions').select('*')
    ...
    let message = `
      ...
      ${
        '- ' +
        contact.value.email +
        (contact.value.message.trim().length > 0 ? '<br>- ' + contact.value.message : '')
      }
    `
